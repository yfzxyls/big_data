...
public class KafkaConsumer {
    ...
    private static String hdfsBasicPath = "hdfs://hadoop200:9000/user/soap/applogs/startup/";

    public static void main(String[] args) {
        // 创建配置
        Properties properties = new Properties();
        properties.put("zookeeper.connect", "hadoop200:2182");
        properties.put("group.id", "log_analysis_consumer");
        ...
        try {
            // 获取HDFS文件系统
            fs = FileSystem.get(new URI("hdfs://hadoop200:9000"), configuration, "hadoop");
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (final KafkaStream<byte[], byte[]> stream : consumerMap.get("log_analysis")) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    while (it.hasNext()) {
                        ...
                        long offset = it.next().offset();

                        System.out.println("msg:" + jasonStr + ",partition; " + partition + ",offset;" + offset);
...
//
                        if (jasonStr.contains("appVersion")) {
                            //System.out.println(Thread.currentThread().getId() + ": startupPage");
                            //save(jasonStr);
...
//        KafkaStream<byte[], byte[]> stream = consumerMap.get("log_analysis").get(0);


    }


    private static synchronized void save(String log) {
        try {
